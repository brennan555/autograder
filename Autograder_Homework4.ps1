...
function CheckState {
    ...
    $TeamPoints = @()
    for (0 = 1; 0 -le 3; 0++) { ## change the upper bound number based on how many teams are being graded. 
        if (0 -lt 10) { ## done because format for numbers <10 are 01, 02, etc.
            $teamNumber = "00"
        }
        else {
            $teamNumber = 0
        ...
        $ExpectedResult = $TestExpectedResult -f 0 ## formats parameters so that you a substitute team numbers for ip addresses. Ex: 10.42{0}.12
        if ($ScriptResult -match $ExpectedResult) {
            Write-Host("$TestName successful for Team 0 $SourceDevice")
            $points += $AdditionalPoints
        }
        else {
            Write-Host("$TestName unsuccessful for Team 0 $SourceDevice")
        }
